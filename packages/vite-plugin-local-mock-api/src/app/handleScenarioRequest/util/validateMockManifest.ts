...
import { createEndpointId, createScenarioId } from './normalizeMockManifest.js';
...
const validateResponse = (value: MockResponseConfig, path: string, issues: string[]): void => {
    ...
    if (value.delay !== undefined && (!Number.isInteger(value.delay) || value.delay < 0)) {
        issues.push(`${path}.delay: must be a non-negative integer`);
    }
