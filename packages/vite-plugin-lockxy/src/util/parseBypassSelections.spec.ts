...
describe('parseBypassSelections', () => {
    ...
    it('recognizes the encoded global bypass value', () => {
        expect(parseBypassSelections('theme=dark; lockxy-bypass=%2A')).toEqual({
            all: true,
            endpointIds: new Set(),
        });
    });
    ...
    it('parses valid endpoint ids and ignores malformed values', () => {
        expect(parseBypassSelections('lockxy-bypass=orders%7Cuser_details%7Cbad%3Aid%7Corders')).toEqual({
            all: false,
    ...
    it('ignores a malformed encoded value', () => {
        const onError = vi.fn();
        expect(parseBypassSelections('lockxy-bypass=%E0%A4%A', onError)).toEqual({
            all: false,
            endpointIds: new Set(),
        });
