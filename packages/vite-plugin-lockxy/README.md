...
An omitted endpoint `method` matches every method. An omitted `file` falls back to naming conventions. A single scenario is selected automatically; multiple scenarios can be selected through the optional panel or scenario cookie. Selected scenario values override endpoint values, endpoint values override the root delay, and remaining file lookup follows naming conventions. Responses with status `204` or `304`, as well as all `HEAD` responses, never include a body.
